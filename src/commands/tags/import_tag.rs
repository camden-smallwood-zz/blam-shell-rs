...
impl ImportTagCommand {
    pub fn new(cache_context: Rc<RefCell<CacheContext>>) -> Self {
        Self { cache_context }
    }
}

impl Command for ImportTagCommand {
    fn get_name(&self) -> &'static str { "ImportTag" }
            ...
            match if args[0].len() >= 2 && &args[0].as_str()[0..2] == "0x" {
                ...
                Ok(tag_index) => {
                    if tag_index < 0 || tag_index >= self.cache_context.borrow().tag_cache.get_tag_count() {
                        ...
                        tag_cache.import_tag_data(tag_index as usize, &mut File::open(args[1].as_str())?)
                    }
                }
            }
        }
